...
function ReportPipeline({ reportId, onComplete }) {
  ...
  return (
    <div className={`wr-pipeline wr-pipeline-${robotState}`}>
      ...
      {/* Avisos (emails ausentes, etc.) */}
      {isCompleted && status?.metadata?.warnings?.length > 0 && (
        <div className="wr-pipeline-warnings">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div>
            {status.metadata.warnings.map((w, i) => (
              <p key={i} style={{ margin: '2px 0', fontSize: '0.85rem' }}>{w}</p>
            ))}
          </div>
        </div>
      )}
