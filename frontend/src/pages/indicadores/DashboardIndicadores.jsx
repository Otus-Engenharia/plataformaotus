...
export default function DashboardIndicadores() {
  ...
  // Ocultar indicadores sem meta configurada (mantém visíveis os que tem meta mas ainda não iniciaram no período)
  const indicadoresVisiveis = indicadoresNoCiclo.filter(ind => {
    const acc = accumulatedMap[ind.id];
    if (!acc) return false;
    if (ind.auto_calculate === false) return true;
    if (acc.planejado > 0) return true;
    // Indicador tem meta mas ainda não iniciou neste mês (mes_inicio > mes_atual)
    if ((parseFloat(ind.meta) || 0) > 0) return true;
    return false;
  });
  ...
  return (
    <div className="dashboard-ind">
      ...
      <section className="indicators-section">
        ...
        {indicadoresOcultos > 0 && indicadoresVisiveis.length > 0 && (
        ...
        {indicadoresVisiveis.length === 0 && indicadoresOcultos === 0 ? (
          ...
          <div className="empty-state">
            ...
            <p className="empty-state__description">
              {indicadoresOcultos} indicador{indicadoresOcultos !== 1 ? 'es' : ''} encontrado{indicadoresOcultos !== 1 ? 's' : ''}, mas sem metas mensais para {getCycleLabel(ciclo)} de {ano}. Solicite ao admin para verificar os templates.
            </p>
          </div>
        ) : (
