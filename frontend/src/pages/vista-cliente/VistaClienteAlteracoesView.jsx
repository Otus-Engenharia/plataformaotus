/**
 * Vista do Cliente - Alterações
 *
 * Página dedicada ao log de alterações mensais do projeto.
 * Mostra histórico de mudanças mês a mês + tabela de impacto por disciplina.
 * Filtra disciplinas "Cliente" e "Coordenação" (não fazem sentido para o cliente).
 */
...
import './VistaClienteAlteracoesView.css';
...
function VistaClienteAlteracoesView() {
  const {
  ...
  const { filteredPairs, displayPairs, disciplineScores, summary } = useMemo(() => {
    ...
    // 1. Filter out invisible changes + apply type filter
    ...
    // 3. Filter discipline_scores (remove client/coordination) and sort by days desc
  ...
  const maxImpact = useMemo(() => {
    if (!disciplineScores.length) return 1;
    return Math.max(...disciplineScores.map(d => d.total));
  }, [disciplineScores]);
    ...
    <div className="vista-cliente-container">
        ...
        <div className="vc-header-title-row">
          {selectedProject ? (
            <h1 className="vc-project-title">
              {selectedProject.project_name || selectedProject.project_code_norm} — Alterações
            </h1>
          ) : (
            <h1 className="vc-project-title">Alterações</h1>
          )}
        </div>
      ...
      {!selectedProjectId ? (
        <div className="vc-empty-state">
          ...
          <div className="vc-empty-state-text">Selecione um projeto para visualizar as alterações.</div>
        </div>
      ) : changeLogLoading ? (
        <div className="vcalt-loading">Carregando log de alterações...</div>
      ) : !changeLog || !changeLog.month_pairs || changeLog.month_pairs.length === 0 ? (
        <div className="vc-empty-state">
          <div className="vc-empty-state-icon">&#128196;</div>
          <div className="vc-empty-state-text">
        ...
        <>
          ...
          <div className="vcalt-content-grid">
            ...
            {/* Right: Discipline Sidebar — Chart first, then table */}
...
export default VistaClienteAlteracoesView;
